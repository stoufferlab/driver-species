...
for(site in levels(complete_network$Site)){
	...
	edge_list<-data.frame(plants=numeric(0),insects=numeric(0))
	...
	write.table(edge_list, file = paste0(site,"_binary" quote = FALSE, row.names = FALSE, col.names = FALSE)
...
main_info_matchings<-data.frame("site"=character(12), "number_links"=numeric(12), "matching_size"=numeric(12), stringsAsFactors=FALSE)
line_number<-1
for(site in levels(complete_network$Site)){
...
write.table(main_info_matchings, file = "binary_matchings_info", quote = FALSE, row.names = FALSE)