make_fig_all_matchings <- function(en_direction, pdf_out = NULL){
  ...
  n2 %<>%
    ...
    ntw_format_theme() %>%
    add_property(element = "vertex", attr_name = "size",attr_base = "type", "TRUE ~ 45") %>%
    ...
    add_property(element = "vertex", attr_name = "shape", attr_base = "type", "TRUE ~ 'rectangle'")
  ...
  n4 <- cliques %>%
    ...
    purrr::map(igraph::set_vertex_attr, name ="size", value = 70)
  
  heading_height <- 0.25
  fig_height <- 2 
  heights <- c(heading_height, fig_height/2, fig_height/2)
  
  total_width <- fig_sizes()$two_column_width
  x <- (total_width - fig_height/2)/3
  widths <- c(x, x, x, fig_height/2)
  ...
  l <- c(rep(1:5, 2), rep(2:1, each = 5)) %>% matrix(ncol = 2) %>%
    rescale_layout(xlim = c(-1, 1)* 0.75, ylim = c(-1, 1)* 0.5)
  ...
  # titles
  standalone_text("P: alternative bipartite representation", font = 2)
