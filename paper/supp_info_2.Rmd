...
We therefore investigated the effect of calculating the dependencies using visitation or pollination effectiveness and importance---two metrics more proximate to plant reproductive success (Figure \@ref(fig:fig-visitation-vs-deposition-vs-importance)).
...
```{r fig-f-s-rankings, echo = F, message = F, warning = F, fig.width = 6.2, fig.height = 9, results = "hide", fig.cap="**Frequency with which a species was classified as a superior node ($f_S$) as a function of the matching weight threshold**. Rows correspond to study sites."}
...
p <- c("f_s", "f_d") %>%
	plyr::llply(function(x){
		threshold_data %>% 
			...
			# dplyr::group_by() %>% View
			ggplot(aes_string(x = "threshold", y = x)) +
			geom_line(aes(group = species), size = 0.2) +
			# geom_point(size = 0.5) +
...
```{r fig-f-d-rankings, echo = F, message = F, warning = F, fig.width = 6.2, fig.height = 9, results = "hide", fig.cap= "**Frequency with which a species was classified as a superior node ($f_D$) as a function of the matching weight threshold**. Rows correspond to study sites."}
...
```{r table-correlations-thresholds, echo = F, message = F, warning = F, results = "asis", sanitize = F}
...
c_m <- round(correlation_matrix_o, 2)
c_m[is.na(c_m)] <- "—"
