...
In the main text we show all maximum cardinality matchings for a example network in which the direction of control is based on the dependence asymmetries (Figure 1). For completeness, here we show all the maximum cardinality matchings for an example network in which the links are bidirectional (Supporting Figure 3). 

Empirical network example here.

It is important to note that our approach to find all the maximum matchings, which is based on the alternative bipartite representation of the directed network and a series of network transofrmations, has an important disadvantage. Namely, that it is possible tha cycles in network figure 4

```{r fig-matchings-bidirectional, echo = F, message = F, warning = F, fig.width = 6.2, fig.height = 3.2, results = "hide", fig.cap = "SAD"}
par(mfrow=c(2,3), mar=c(0,0,0,0))
...
```{r  fig-matchings-bidirectional-no, echo = F, message = F, warning = F, fig.width = 6.2, fig.height = 4.8, results = "hide", fig.cap= "asd asd"}
