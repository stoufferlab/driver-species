...
%DIF DEL drafts_and_revisions/2018-07-19/manuscript.tex   Sun Jan 13 23:48:09 2019
%DIF ADD manuscript.tex                                   Sun Jan 13 23:49:46 2019
\usepackage{lmodern}
...
\item
  An important dimension of a species\DIFaddbegin \DIFadd{' }\DIFaddend role is its ability to alter the
  ...
  species. While centrality is useful when predicting which species\DIFaddbegin \DIFadd{'
  }\DIFaddend extinctions could cause the largest change in a community, it says
...
\item
  ...
  to achieve a desired state. \DIFaddbegin \R{R2-define-control-capacity} \DIFaddend It also
  allows us to quantify a species\DIFaddbegin \DIFadd{' }\DIFaddend control capacity---its relative
  importance---and identify the set of species that, collectively, are
  critical in this context \DIFaddbegin \DIFadd{because they have largest possible control
  capacity}\DIFaddend . We illustrate the application of structural controllability
  with ten pairs of uninvaded and invaded plant-pollinator communities.
\item
  ...
  \DIFaddbegin \R{R2-our-dataset} \DIFaddend Interestingly, this set of critical species was
  mostly composed of plants and \DIFaddbegin \DIFadd{included }\DIFaddend every invasive species in our
  dataset\DIFdelbegin \DIFdel{was part of it}\DIFdelend . We also found that species with high control capacity, and in
  particular critical species, contribute the most to the stable
  coexistence of their community. \DIFaddbegin \R{R2-species-degree} \DIFaddend This result was
  true, even when controlling for \DIFdelbegin \DIFdel{its
  }\DIFdelend \DIFaddbegin \DIFadd{the species' }\DIFaddend degree,
  abundance/interaction strength, and the relative dependence of their
  partners.
\item
...
\caption{\label{tab:tab-glossary}\DIFaddFL{Glossary}}
\centering
\fontsize{8}{10}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{3.4in}l}
\toprule
\textbf{\DIFaddFL{network control}} & \\
\DIFaddFL{\hspace{1em}A network is said to be controllable if it is possible to steer it from an initial to an arbitrary final state within finite time. }& \\
\addlinespace
\textbf{\DIFaddFL{controllability}} & \\
\DIFaddFL{\hspace{1em}The intrinsic difficulty of controlling an ecological community. It is measured by the relative size of the minimum driver-node set, $n_D$. It also indicates the extent to which network structure can be harnessed for network control. }& \\
\addlinespace
\textbf{\DIFaddFL{minimum driver-node set}} & \\
\DIFaddFL{\hspace{1em}One of the sets of species whose abundances need to be directly managed in order to achieve full control of the community. The minimum driver-node sets can be obtained by finding all maximum matchings in a network. }& \\
\addlinespace
\textbf{\DIFaddFL{maximum matching}} & \\
\DIFaddFL{\hspace{1em}A matching is a set of links that do not share any common start or end nodes; the largest possible matching is called a maximum matching. }& \\
\addlinespace
\textbf{\DIFaddFL{control configuration}} & \\
\DIFaddFL{\hspace{1em}One of the species combinations with which is possible to achieve network control. Optimal control configurations are given by the minimum driver-node sets. }& \\
\addlinespace
\textbf{\DIFaddFL{control capacity}} & \\
\DIFaddFL{\hspace{1em}The relative frequency \(\phi\) which with a species is part of the optimal control configurations of a network. }& \\
\addlinespace
\textbf{\DIFaddFL{critical species}} & \\
\DIFaddFL{\hspace{1em}A species with a maximal control capacity \(\phi=1\) }& \\
\addlinespace
\textbf{\DIFaddFL{superior node}} & \\
\DIFaddFL{\hspace{1em}A species is a superior node if it can internally affect the abundance of other species in the network. Superior nodes make up the chains that propagate the control signals through the network. }& \\
\bottomrule
\end{tabular}
\end{table}
...
\DIFdelbegin \DIFdel{Our first ecological }\DIFdelend \DIFaddbegin \R{R2-ecol-objective} \DIFadd{Our first }\DIFaddend objective is to investigate whether the
controllability of a community is associated with invasion status or
not. \DIFaddbegin \R{R2-desired-control-trajectory} \DIFaddend Finding out exactly how difficult
it is to control a network depends strongly on the particularities of
the desired control trajectory \DIFaddbegin \DIFadd{(i.e.~the path to the desired final
state) }\DIFaddend as well as the dynamical relationship between nodes. However, we
are interested in understanding the controllability of a network in a
...
Fiala, \& Blüthgen, 2007)\DIFaddbegin \R{R2-citation-with-name}\DIFaddend . We compared models
using the Akaike Information Criterion for small sample sizes (AICc).
...
\DIFaddbegin \R{R2-one-lumped-model-a}\DIFaddend We then examined how species-level properties
were related to control capacity using a set of \DIFaddbegin \DIFadd{candidate }\DIFaddend generalised
linear models with binomial error structure. These models included five
predictor variables that mirror the network-level predictors. First, the
species\DIFaddbegin \DIFadd{' }\DIFaddend contribution to nestedness, which has been proposed as a key
feature that promotes stability and robustness in mutualistic networks
(\DIFdelbegin \DIFdel{S. }\DIFdelend Saavedra, Stouffer, Uzzi, \& Bascompte, 2011).
Second,\DIFdelbegin \DIFdel{the species }\DIFdelend \DIFaddbegin \R{R2-species-strength-a} \DIFadd{the species' }\DIFaddend strength (the sum of a
species' visits), which quantifies the strength of a species\DIFaddbegin \DIFadd{'
}\DIFaddend associations and is indirectly related to its abundance (Poisot, Canard,
...
(\DIFdelbegin \DIFdel{Diego P }\DIFdelend Vázquez et al., 2007). Fourth, the species\DIFaddbegin \DIFadd{' }\DIFaddend degree in order to account
for the intrinsic centrality of a species. Finally, we included a
categorical variable for the species\DIFaddbegin \DIFadd{' }\DIFaddend trophic level (plant or
...
deviation of one. \DIFaddbegin \R{R2-one-lumped-model-b}\DIFadd{In these models, species from
all networks were lumped together. }\R{R2-random-effects}\DIFadd{We initially
included random effects to account for possible variation across
communities, but we found that the variance explained by these random
effects was zero, and therefore random effects were not included in
further analyses. }\R{R2-all-candidates}\DIFadd{We then generated all possible
candidate models. }\DIFaddend To identify the models that were best supported by the
...
In addition, we wanted to understand how a species\DIFaddbegin \DIFadd{' }\DIFaddend control capacity
...
\DIFdelbegin %DIFDELCMD < \R{testing-assumptions-line}%%%
\DIFdelend Just like the centrality metrics, the information obtained by applying
...
Newstrom-Lloyd, Potts, \& Dafni, \DIFdelbegin \DIFdel{2009). More
details in the }\DIFdelend \DIFaddbegin \DIFadd{2010). }\R{R2-sentence-fragment} \DIFadd{See
}\DIFaddend Supporting Information Section \ref{S-visitation-as-proxy} \DIFaddbegin \DIFadd{for more
details}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \R{testing-assumptions-results-line} %%%
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{tab:tab-glossary}%%%
\DIFdelFL{Glossary}}
%DIFAUXCMD
%DIFDELCMD < \centering
%DIFDELCMD < \fontsize{8}{10}\selectfont
%DIFDELCMD < \begin{tabular}[t]{>{\raggedright\arraybackslash}p{3.4in}l}
%DIFDELCMD < \toprule
%DIFDELCMD < %%%
\textbf{\DIFdelFL{network control}} %DIFAUXCMD
%DIFDELCMD < & \\
%DIFDELCMD < %%%
\DIFdelFL{\hspace{1em}A network is said to be controllable if it is possible to steer it from an initial to an arbitrary final state within finite time. }%DIFDELCMD < & \\
%DIFDELCMD < \addlinespace
%DIFDELCMD < %%%
\textbf{\DIFdelFL{controllability}} %DIFAUXCMD
%DIFDELCMD < & \\
%DIFDELCMD < %%%
\DIFdelFL{\hspace{1em}The intrinsic difficulty of controlling an ecological community. It is measured by the relative size of the minimum driver-node set, $n_D$. It also indicates the extent to which network structure can be harnessed for network control. }%DIFDELCMD < & \\
%DIFDELCMD < \addlinespace
%DIFDELCMD < %%%
\textbf{\DIFdelFL{minimum driver-node set}} %DIFAUXCMD
%DIFDELCMD < & \\
%DIFDELCMD < %%%
\DIFdelFL{\hspace{1em}One of the sets of species whose abundances need to be directly managed in order to achieve full control of the community. The minimum driver-node sets can be obtained by finding all maximum matchings in a network. }%DIFDELCMD < & \\
%DIFDELCMD < \addlinespace
%DIFDELCMD < %%%
\textbf{\DIFdelFL{maximum matching}} %DIFAUXCMD
%DIFDELCMD < & \\
%DIFDELCMD < %%%
\DIFdelFL{\hspace{1em}A matching is a set of links that do not share any common start or end nodes; the largest possible matching is called a maximum matching. }%DIFDELCMD < & \\
%DIFDELCMD < \addlinespace
%DIFDELCMD < %%%
\textbf{\DIFdelFL{control configuration}} %DIFAUXCMD
%DIFDELCMD < & \\
%DIFDELCMD < %%%
\DIFdelFL{\hspace{1em}One of the species combinations with which is possible to achieve network control. Optimal control configurations are given by the minimum driver-node sets. }%DIFDELCMD < & \\
%DIFDELCMD < \addlinespace
%DIFDELCMD < %%%
\textbf{\DIFdelFL{control capacity}} %DIFAUXCMD
%DIFDELCMD < & \\
%DIFDELCMD < %%%
\DIFdelFL{\hspace{1em}The relative frequency \(\phi\) which with a species is part of the optimal control configurations of a network. }%DIFDELCMD < & \\
%DIFDELCMD < \addlinespace
%DIFDELCMD < %%%
\textbf{\DIFdelFL{critical species}} %DIFAUXCMD
%DIFDELCMD < & \\
%DIFDELCMD < %%%
\DIFdelFL{\hspace{1em}A species with a maximal control capacity \(\phi=1\) }%DIFDELCMD < & \\
%DIFDELCMD < \addlinespace
%DIFDELCMD < %%%
\textbf{\DIFdelFL{superior node}} %DIFAUXCMD
%DIFDELCMD < & \\
%DIFDELCMD < %%%
\DIFdelFL{\hspace{1em}A species is a superior node if it can internally affect the abundance of other species in the network. Superior nodes make up the chains that propagate the control signals through the network. }%DIFDELCMD < & \\
...
networks. \DIFdelbegin \DIFdel{instead }\DIFdelend \DIFaddbegin \DIFadd{Instead }\DIFaddend controllability is almost completely constrained by
the patterns of species richness at each trophic guild and their degree
distributions (N. Blüthgen et al., 2007; \DIFdelbegin \DIFdel{C. J. }\DIFdelend Melián \& Bascompte, 2002).
...
(Astegiano, Massol, Vidal, Cheptou, \& Guimarães, 2015; \DIFdelbegin \DIFdel{J. }\DIFdelend Bascompte et
al., 2006; \DIFdelbegin \DIFdel{J. }\DIFdelend Memmott, Waser, \& Price, 2004).

\DIFaddbegin \R{R2-dynamic-persistence}\DIFaddend Accordingly, our results suggest that
structural controllability is closely related to the \DIFdelbegin \DIFdel{dynamic }\DIFdelend persistence of an
...
robustness to extinction (Bastolla et al., 2009; \DIFdelbegin \DIFdel{J. }\DIFdelend Memmott et al., 2004)
even at the expense of the dynamic stability of the mutualistic
community (\DIFdelbegin \DIFdel{S.
}\DIFdelend Saavedra et al., 2016). These observations are in agreement
with our results, as we would expect the dynamic stability
\DIFaddbegin \R{R2-dynamic-stability}\DIFadd{(the ability to return to equilibrium after a
perturbation in species abundances) }\DIFaddend of a community to be correlated to
...
When controlling for a \DIFdelbegin \DIFdel{species' strength}\DIFdelend \DIFaddbegin \R{R2-species-strength-b}\DIFadd{species' visitation
strength (the sum of a species' visits)}\DIFaddend , which is indirectly a proxy of
...
species extinctions (Jordan, 2009). However, further \DIFdelbegin \DIFdel{, }\DIFdelend inspection shows
...
\DIFdelbegin %DIFDELCMD < \R{management-implications}%%%
\DIFdelend Other conceptual differences between structural controllability and
centrality metrics provide three key insights into the conservation of
ecological networks. \DIFaddbegin \R{R2-structural-controllability} \DIFaddend First, structural
controllability emphasizes that the effect a species has on the
abundance of other species is not independent of the effects of \DIFdelbegin \DIFdel{other
other }\DIFdelend \DIFaddbegin \DIFadd{those
other }\DIFaddend species in their community. The rankings provided by centrality
...
species\DIFdelbegin \DIFdel{control capacity}%DIFDELCMD < \R{extra-references-1} %%%
\DIFdelend \DIFaddbegin \DIFadd{' control capacity }\DIFaddend (Devoto, Bailey, Craze, \& Memmott, 2012;
...
ranking-based approaches \DIFdelbegin %DIFDELCMD < \R{extra-references-2} %%%
\DIFdelend (Harvey, Gounand, Ward, \& Altermatt, 2017).
...
\& Englund, 2016) \DIFdelbegin \DIFdel{and to be central }\DIFdelend \DIFaddbegin \R{R2-central-in-their-communities}\DIFadd{and to attain high
centrality }\DIFaddend in their communities (Palacio, Valderrama-Ardila, \& Kattan,
2016; Vila et al., 2009). \DIFaddbegin \R{R2-inherrently-different} \DIFaddend We found that \DIFdelbegin \DIFdel{invasive plants are, however, unlikely to be inherently different from
}\DIFdelend \DIFaddbegin \DIFadd{it
is not that invasive plants have some different mechanism for
influencing the community compared to }\DIFaddend their native counterparts (Emer,
Memmott, Vaughan, Montoya, \& Tylianakis, 2016; Stouffer, Cirtwill, \&
Bascompte, 2014). \DIFdelbegin \DIFdel{Just like any
other mutualist in our dataset, }\DIFdelend \DIFaddbegin \DIFadd{Both native species and }\DIFaddend invasive plants tended to
attain a high control capacity \DIFdelbegin \DIFdel{proportional to the degree to which they contribute }\DIFdelend \DIFaddbegin \DIFadd{if they were important }\DIFaddend to network
persistence, \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend abundant, and \DIFdelbegin \DIFdel{depend }\DIFdelend \DIFaddbegin \DIFadd{depended }\DIFaddend little on other species.
...
have transformative desirable effects in plant-pollinator communities
\DIFdelbegin %DIFDELCMD < \R{extra-references-3} %%%
\DIFdelend (Kaiser-Bunbury et al., 2017). However, our results also suggest that
removals must be exercised with caution. Not only it is hard to predict
the direction in which the system will change, but we also show that
critical species can underpin the coexistence of species and therefore
some communities may be acutely vulnerable to their eradication
(Albrecht, Padron, Bartomeus, \& Traveset, 2014; Traveset et al., 2013).
...
incorporate the species\DIFaddbegin \DIFadd{' }\DIFaddend dynamics into the controllability analysis
(Cornelius, Kath, \& Motter, 2013); the reality is that such information
is rarely available in most ecological scenarios. \DIFdelbegin %DIFDELCMD < \R{data-requirements}%%%
\DIFdelend In contrast,
structural controllability only requires a quantitative approximation of
the network's interactions to gain valuable insight from the community.
Furthermore, while the relationship between centrality and keystoneness
is based on an intuitive understanding of what a keystone species is,
the assumptions of structural controllability are explicit and the
estimation of a species\DIFaddbegin \DIFadd{' }\DIFaddend importance arises from a mechanistic
understanding of the population dynamics between species.
\DIFaddbegin \R{R2-marginally-affected} \DIFaddend By accounting for network dynamics (even if
in a simple way), structural stability incorporates more ecological
realism, especially in the extreme scenario in which the \DIFdelbegin \DIFdel{state of
a community is only marginally affected by the
topology of
their interactions}\DIFdelend \DIFaddbegin \DIFadd{structure of
interactions within the community only marginally affects the
community's state}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \hypertarget{ref-bartomeus_contrasting_2008-1}{}
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{ref-neeman_framework_2009}{}
%DIFDELCMD < %%%
