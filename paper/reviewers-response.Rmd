...
<!-- FDO: The 'probability/likelihood of being a superior node' is not mentioned in the main text, not even once. It only briefly gets mentioned in the Results and the Discussion and that is more in the conceptual way of what it means to be superior node. Not on how we devise this 'likelihood' when we have multiple non-reciprocal instances of a network. Plus in the paper, we don't really get much ecological insight from it. Because it's only mentioned in the supp. info I added this there and I think that'll be fine. We can always change it if they come back to it -->
<!-- DBS: Agreed. -->
...
<!-- DBS: What I mean is what you said here in the response but not the text: what is meant by groups in the random effect model. So when you say possible variation across communities you should make explicit that this was done in some number of different ways, one with a random effect for network, one with a random effect for site, etc. We never specify the EXACT random effect structure (and I can only partially decipher it from the paragraph here), so "variation across communities" can be interpreted in a large number of ways.-->
...
<!-- FDO: Humm. I cannot find a good place to put this in the main text without having to write a half/full paragraph about it... -->
<!-- DBS: Why not somewhere in line 172 (in the Rmd file)? You could replace "Sometimes" with "For X% of the observed species pairs, there is no asymmetry..." -->
