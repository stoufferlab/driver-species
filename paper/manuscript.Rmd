...
formated_en_structural <- en_structural %>%
...
# 3-6
for (i in 2:length(formated_en_structural)){
  set.seed(5)
  types <- igraph::V(formated_en_structural[[i]])$type ==
    dplyr::first(igraph::V(formated_en_structural[[i]])$type)
  l <- igraph::layout_as_bipartite(formated_en_structural[[i]], types) %>%
    rescale_layout(xlim = c(-1.75, 1.75), ylim = c(-0.6, 0.6))
  plot_examples(formated_en_structural[[i]], 
                vertex.size = 45,
                layout = l,
                frame = F, 
                rescale = F, 
