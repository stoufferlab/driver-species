...
Finally, we wanted to understand how the control capacity $\phi$ and the probability of being a superior node $\sigma$ is related to metrics of keystoness based on centrality. 
...
To do that we compared the results obtained when using the full network and when randomly removing interactions preferentially from the weakest links in the network. 
...
Indeed the null model ($n_D \sim 1$) was the most parsimonious candidate model and all variables were relatively unimportant (based on the AICc; Table  \@ref(S-tab:tab-controllability-model-results)). 
...
Specifically, species with a positive asymmetry (this is they affect their partners more than what their partners affect them), and with a large visitation strength were more likely to be superior nodes (Figure \@ref(fig:fig-species-level)c and d). 
Furthermore, species degree was positively, but only weakly associated with both control capacity and the probability of being a superior node. 
...
We found that using visitation as a proxy for the strength of species interactions leads to similar results than those obtained using pollinator importance [regarded as an accurate measure of the pollination service to plants; Supporting Information Section \@ref(S-visitation-as-proxy); @neeman_framework_2009]. 
