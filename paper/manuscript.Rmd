...
We used ten paired pollination communities to test the control theoretic approach.
...
The first step before applying methods of control theory is to construct a directed network that can provide an indication of the extent to which species affect each others abundance. 
In some ecological networks, establishing the directionality can appear relatively straightforward, for example when links represent biomass transfer or energy flow [@isbell_human_2013]. 
...
Strictly speaking, a matching in an unweighted directed graph is defined as a set of links that do not share a common start or end nodes, a node is matched if its at the end of a matched link and unmatched if not, and the largest possible matching is called maximum  matching. 
Maximum matching is a widely studied topic in graph theory and has been used in multiple real life applications, from dating apps and wireless communications to transplant donations and peer to peer file sharing.
Although seemingly unrelated, maximum matching is also connected to the controllability of a network because, as recently shown [@liu_controllability_2011], a matching can be decomposed into a set of paths that reveal how the control signal flows across the links in a network to reach every node that compose it. 
Unmatched nodes are the beginning of 
The result is that the minimum driver node set corresponds exactly to the *unmatched* nodes, those that which the control signal 

to can be decomposed into a set of paths


A maximum matching is this. 
And can be calculatid this way. 
And gives this information. 
But its not unique.
This can provide extra information. 
Therefore we calculate them all. 
