...
Indeed the null model ($n_D \sim 1$) was the most parsimonious candidate model and all variables were relatively unimportant (based on the AICc; Table  \@ref(S-tab:tab-controllabillity-model-results)). 
...
The models that best explained a species control capacity $\phi$, had a random intercept and slope for the species asymmetry grouped by guild (plants or pollinators; Table \@ref(S-tab:tab-species-model-random), Table \@ref(S-tab:tab-model-output-species-level)). 
...
For the probability of being a superior node $\sigma$, the most parsimonuous models did not include any random effect Table \@ref(S-tab:tab-species-model-random), Table \@ref(S-tab:tab-model-output-species-level)). 
...
Although some pollinators had large values of control capacity, the group of critical species in every community was composed exclusivelly by plants. 
...
On the other hand, the probability of being a superior node $\sigma$ was slightly more similar to established centrality metrics. 
...
We found that using visitation as a proxy for the strenghth of species interactions leads to similar results than those obtained using pollinator importance [regarded as an accurate measure of the pollination service to plants; Supporting Information Section \@ref(S-visitation-as-proxy); @neeman_framework_2009]. 
