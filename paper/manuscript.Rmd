...
In contrast, there was an important negative relationship between $n_D$ and the network asymmetry (Figure \@ref(fig:fig-emp-controllability)b).
<!-- DBS: "important" doesn't mean anything statistically. was it significant? -->
...
Pollinators had, in average, larger control capacities than plants. That said, almost no pollinator was critical for network control, where a species is critical for control if it has control capacity \(\phi_i = 1\)). 
...
<!-- DBS: y-axis in (a) should be "probability density" -->
<!-- DBS: I also *love* this plot. -->
...
# dev.off()
```
<!-- DBS: You never state in the methods what you mean by model-average partial residuals. If you don't explain it there, you MUST explain it here as there is a good chance that no one will invest the time required to know what is actually going on. -->
...
We also found that critical species have a particularly large impact on species coexistence when compared to non-critical species.
...
<!-- DBS: I wonder if people will have a hard time understanding what these values mean. That is, the differences are clear visually but what do those values of Omega actually mean? I am almost certain a reviewer is going to nag about this. -->

<!-- DBS: y-axis should be "probability density" -->
<!-- DBS: remove the title at the top of the figure -->
<!-- DBS: the issue of scaling of Omega should likely appear in the methods too... -->
```{r fig-structural-stability, fig.width=fig_sizes()[[1]], fig.height=fig_sizes()[[1]]/2, fig.cap="Probability density of the structural stability of the communities after a single focal species is removed. The structural stability after critical species have been removed (darker line) is considerable smaller than that after non-critical (redundant) species have been removed. To allow comparison across communities, the structural stability values were scaled within each network to have a mean of zero and a standard deviation of one."}
...
The Spearman correlation between these ranged between `r round(min(correlations$control_capacity), 2)` (with betweeness-centrality) and `r round(max(correlations$control_capacity), 2)` (with eigen-centrality). 
On the other hand, the probability of being a superior species $\sigma$ was slightly more correlated to established centrality metrics as its Spearman correlation ranged between `r round(min(correlations$superior), 2)` (with eigen-centrality) and `r round(max(correlations$superior), 2)` (with degree; Figure \@ref(S-fig:fig-correlogram)). 
...
<!-- DBS: Please rephrase this last sentence and/or chop it into two. It's unclear to me (at least) in particular the "with the exception" part. -->
...
<!-- DBS: Two sentences does not make a paragraph.... Please add one more that summarizes what we found. -->
...
Nevertheless, structural controllability might be helpful at identifying communities in which changes in the ecological state are more likely to occur and provides solid theoretical rationale for the many difficulties encountered in the management and restoration of natural communities [@addrefs].
...
These results suggest that the differences between the controllability across networks are instead likely to arise from differences in their structure. 
...
Accordingly, our results suggest that structural controllability is closely related to the dynamic persistence of an ecological community. 
First, we found a comparatively small but thought-provoking negative relationship between the controllability of a network and the its nestedness. 
...
Without exception, we found that invasive plants were critical for network control and as such our results were in line with our expectations. 
...
Structural controllability assumes that the networks can be approximated using linear functional responses [@addrefs]. 
...
<!-- DBS: See what everyone else says about these last two sentences. I think they get harder and harder to follow as they evolve. -->
...
While useful, centrality metrics, which have often been used as a proxy for keystoneness, fail to identify some of these species which highlights their limitations when we fully embrace the notion that ecological communities are dynamical systems.
...
<!-- DBS: Surely we had this before. Please copy it from a previous submission. And definitely DON'T submit without the right acknowledgement text! -->
...
<!-- DBS: Surely we had this before. Please copy it from a previous submission. -->
