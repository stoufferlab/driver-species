...
```{r fig-example-networks, fig.width=fig_struct_control$width, fig.height=fig_struct_control$height, fig.cap="(a) To establish the direction of control we start with a weighted visitation network (on the right). In this network, the width of the links correspond to the frequency of visitation. We then calculate the dependencies between species pairs and create a directed network (left) in which the direction of the links correspond to the direction of the assymetry of their dependence. For instance, the population of plant $p_1$ is likely to be affected by the population of pollinator $a_1$ to a larger extent that the population of $a_1$ depends on $p_1$ because $a_1$ interacts more strongly with other species in the community; hence the direction of control between these two species can be simplyfied with a directed link from $a_1$ to $p_1$. (b) A system is structurally controllable if there are no inaccessible links or dilations. The network on the bottom left is not controllable because there is a dilation in which node $x_2$ is being used to control two nodes simultaneously. The network in the bottom right  is not controllable because node $x_3$ is inaccessible from the input node $u_1$ in the system. Both systems can be made controllable by adding an extra input node (top)."}
...
For example, in a network composed by jobs and job applicants, a matching is any pairing between applicants and positions that satisfies the basic constrain of any applicant being assigned to at most one position and visceversa.
A maximum matching would then be such a pairing that maximises the number of applicants with jobs and the number of positions filled. 
The definition of maximum matching, however, offer little insight about how it is related to structural controllability. 
The key is to note that the conditions of structural controllability imply that there needs to be a one to one relationship between *superior* and *subordinate* nodes (Figure \@ref(fig:fig-example-networks)b, bottom left). 
We use the maximum matching algorithm to pair superior to subordinate nodes in a way consistent with these conditions. 
...
And so, as recently shown [@liu_controllability_2011], the minimum driver node set, those to which an external control signal should be applied to make the system controllable, corresponds exactly to the *unmatched* nodes in the network (Figure \@ref(fig:fig-control-configuration)).

```{r}
drake::loadd(fig_control_config)
```

```{r fig-control-configuration, fig.height=fig_control_config$height, fig.width=fig_control_config$width,fig.cap = "ASD"}
