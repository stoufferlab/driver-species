...
formatted_en_structural <- en_structural %>%
  purrr::map(~ fancify_vertex_name(.)) %>%
  purrr::map(~ add_property(., element = "edge", attr_name = "color", 'TRUE ~ "black"')) %>%
  purrr::map(~ add_property(., element = "edge", attr_name = "lty", "type == 'a' ~ 1", "TRUE ~ 3")) %>%
  ...
  purrr::map(~ add_property(., element = "vertex", attr_name = "size", "type == 'a' ~ 55", "TRUE ~ 45")) %>%
  purrr::map(~ add_property(., element = "vertex", attr_name = "label.cex", "TRUE ~ 1")) %>%
  purrr::map(~ add_property(., element = "vertex", attr_name = "label.color", "TRUE ~ 'black'"))
```
...
In addition, a graphical interpretation of the structural controllability theorem enables us to translate the control problem into a topological one: a network is controllable if there are no inaccessible nodes—nodes without incoming links—or dilations—*expansions* of the network (Figure \@ref(fig:fig-example-networks)b; see supplementary information section \@ref(S-structural-controllability) for more details).
