...
The size of the minimum driver node set relative to the number of species in each network $n_D$ ranged between $n_D = `r round(min(c_m$n_D), 2)`$ and $n_D = `r round(max(c_m$n_D), 2)`$ (mean `r round(mean(c_m$n_D), 2)`, Figure \@ref(fig:fig-emp-controllability)). 
We found that the relative size of the minimum driver node set was not related to any of our independent variables. 
Indeed, as determined by the AICc, the null model was the most parsimonious candidate and all variables were relatively unimportant (Supplementary Information \@ref(S-controllability-models)). 
