...
A particularly relevant dimension of any species' role is its ability to alter the abundance of other species and the state of the community—since changes of this nature can have knock-on effects on ecosystem function, diversity, processes, and services [@ref_to_daniels_tree_paper_and_jasons_reviews].
This ability is sometimes referred to as a species' "keystoneness" [@mills_keystone-species_1993]. 

A significant proportion of the network tools used to estimate species roles in this context rely on the calculation of a species' centrality—a relative ranking of its positional importance that originally stems from social-network research [@noah_e._friedkin_theoretical_1991]. 
...
Centrality metrics have been shown to be useful tools to rank species in regards to their potential to alter the abundances of other species, in particular when estimating secondary extinctions that may follow the loss of a species [@dunne_network_2002; @kaiser-bunbury_robustness_2010].
...
On one hand, substantive changes in ecosystem functioning can also occur without complete removal of a species [@needsrefs;@daniel_has_ideas]. 
...
<!-- DBS: We could potentially cut the sentence above; I see the merit, but I also feel like it could annoy readers/reviewers. -->
...
Species abundances---and consequently the state of the community as a whole---are influenced by both their interactions, the specific dynamics of these interactions and the mechanisms of self-regulation [@addrefs]. 
However, community and population dynamics can be modelled in innumerable ways, and empirical support for one versus another is often still ambiguous [@addrefs]. 
...
At its fundamental level, control theory first determines whether a system is controllable or not; that is, it asks if a system could ever be driven to a desired state within a finite amount of time. 
...
<!-- DBS: I don't get what "more than a ranking" means above. -->
...
Here, we apply methods from control theory to a particular ecological problem and show how it can be used to generate insight into the role of species in an ecological network. 
...
We then expand existing methods from control theory to effectively link the *controllability* of a network with the role of its constituent species.
...
<!-- DBS: Do we need to introduce structural stability before mentioning it here? -->
...
<!-- DBS: Is "supporting the diversity" the right function to call out at this point? -->
...
An intuitive way to understand structural controllability is by looking at its graphical interpretation: from a topological perspective a network is structurally controllable if there are no inaccessible nodes—that is, nodes without incoming links—or dilations—*expansions* of the network (Figure \@ref(fig:fig-example-networks)b; Supporting Information Section \@ref(S-structural-controllability)).
...
<!-- DBS: I think we should at least introduce what superior and subordinate means here even if they're also defined in the figure. -->
...
Our second ecological objective is related to how species differ in their ability to drive the population dynamics of the community.
...
The higher a species control capacity, the greater the likelihood that they would need to be directly managed to change (or maintain) the ecological state of their community. 
Therefore a species control capacity provides an estimation of their relative importance at driving the state of the community [@jia_control_2013]. 
...
Here, we extend this algorithm such that it is possible to calculate a highly accurate approximation of the control capacity $\phi_i$ of every species in the network (Supporting Information Section \@ref(S-input-graph)). 
...
Fourth, the species degree in order to account for the innate centrality of a species.
...
Our analysis revealed that some species have a control capacity $\phi_i = 1$. 
...
Within mutualistic networks, one useful measure of for how networks impact stable coexistence is called structural stability [@rohr_structural_2014]. 
Mathematically, the structural stability $\Omega$ of a network represents the size of the parameter space (i.e., growth rates, carrying capacities, etc.) under which all species can sustain positive abundances [@saavedra_nested_2016]. 
...
<!-- DBS: I think we're far too sloppy about when things like phi have subscripts and when they don't. We really should be consistent throughout. -->
Finally, we wanted to understand how two species-level control metrics are related to five metrics of keystoneness based on centrality. 
For the centrality metrics, we use a species' degree, betweenness, eigen centrality, page rank, and closeness centrality. 
<!-- DBS: You MUST cite where these centrality metrics come from. -->
The first control metric we use is a species control capacity $\phi$ described above. 
...
<!-- DBS: I think that the statement above needs a reference as I don't think this is "new" here nor do I think everyone will necessarily understand why it's true without pointing them to a more formal explanation. -->
...
To enable readers to locate archived data from papers, we require that authors list the database and the respective accession numbers or DOIs for all data from the manuscript that has been made publicly available. Please see our editorial policies page for further information
