...
The models that best explained a species control capacity $\phi$, had a random intercept and slope for the species asymmetry grouped by guild (plants or pollinators; Table \@ref(S-tab:tab-model-output-control-capacity)). 
Contribution to nestedness was the most important variable followed by visitation strength (Table \@ref(tab:tab-model-selection-table), \@ref(S-tab:tab-var-importance)). 
Species with large contribution to nestedness and large visitation strength were more likely to have a large control capacity (Figure \@ref(fig:fig-species-level)a and b). 
For the probability of being a superior node $\sigma$, the most parsimonuous models did not include any random effect (Table \@ref(S-tab:tab-model-output-superior)). 
The most important variables were the species asymmetry and the visitation strength (Table \@ref(tab:tab-model-selection-table), \@ref(S-tab:tab-var-importance)). 
Specifically, species with a positive asymmetry (this is they affect their partners more than what their parners affect them), and with a large visitatin strength were more likely to be superior nodes (Figure \@ref(fig:fig-species-level)c and d). 
Furthermore, degree was positively, but only weakly asociated to both control capacity and the probability of being a superior node. 
Remarkably, many species with a low degree, especially pollinators attained a large control capacity in their communities \@ref(S-fig:fig-species-models-degree).
...
dplyr::bind_rows(cc_table, su_table) %>%
  ...
  knitr::kable("latex", 
               ...
               booktabs = T, 
               caption = "Selection table of the generalised mixed effect models of control capacity $\\phi$ and the probability of being a superior node $\\sigma$. Only models with a weight larger than 0.01 are shown. All variables were scaled and so the effect sizes are comparable.", 
               row.names = F, 
               digits = 2, 
               escape = F) %>%
...
```{r fig-species-level, fig.width=fig_sizes()[[2]]*2/3, fig.height=fig_sizes()[[1]], fig.cap="Relationship between the most important variables in the models of control capacity (a, b) and the probability of being a superior node (c, d). The plots show the values predicted by most parsimonious model for each response variable. The model for control capacity include a random effect for the trophic guild , while the model of superior probability does not include any random effect that distinguish between plants and pollinators. The invasive species are depicted with solid orange circles."}
