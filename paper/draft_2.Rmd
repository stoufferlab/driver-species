...
At the core of control theory of complex networks rests the idea that the state of a node depends on the state of the nodes it interacts with, and the particular form of this dependency is determined both by the dynamic relationship among interacting nodes as well as the structure of the links in the network. 
A large proportion of the recent advances on control theory have focused on linear systems as a first step to understand the controllability of nonlinear dynamics, in fact showing that fundamental can insight about the control of complex systems can be addressed without a detailed knowledge of the nonlinear dynamics and the system parameters [@Liu2015]. 
We therefore focus on complex networks whose dynamics can be locally linearised, at least near equilibium points, to the form $\frac{dx}{dt} = Ax + Bu(t)$, where the change over time of its state ($\frac{dx}{dt}$) depends on its current state *x* (the species' populations in an ecological context), an external time-varying input *u(t)*, and the matrices *A* and *B*, which encapsulate information about the network structure and how the network responds to the external input, respectively. 
Previous research has shown that such a system is controllable if there exists a minimum subset *D* of nodes in the network ($S > D \geq 1$, where *S* is the total number of species) to which to apply an external control signal that will propagate through the network and in turn drive the state of every other node in the network to a desired configuration in finite time [@Kalman1963; @Liu2011]. 
...
We emphasise that, strictly speaking, the network structure tells us whether a linear approximation of the system is controllable or not, but it provides limited information about the characteristics of the control signal or the feasibility of its implementation [@Motter2015].
Despite this limitation, structural controllability provides a strong indication about our ability to control a system without requiring full and precise knowledge of the dynamical relationships among nodes.
...
Moreover, wh show here how quantitative data about species interactions can help us move past some of the limitations of structural controllability by better approximating the role of dynamical relationships without being overly dependent on the specific choices of how these dynamics are modelled or characterised.
...
In an ecological context, external signals that modify the state of a node can be thought of as management interventions.
We therefore use the density of driver nodes $n_D = \frac{D}{S},$ as an index of the manageability of an ecological community.
...
Consequently, we will show how species can be further characterised in terms of the frequency with which they exist on key paths leading through the network, which provides an indication of their relative importance in driving the state of the community. 
...
Furthermore, we focus not only on driver nodes, as is customary in the control literature, but also on superior nodes because realistically, from an ecological perspective, we are not yet able to aim for full control of the abundances of all species in the community.
...
Specifically, we removed links with a probability inversely proportional to its weight effectivelly simulating 
