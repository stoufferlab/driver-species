...
(FDO revision) First, as species with a high *f~S~*, changes on their abundance have the potential to propagate broadly through the community and in turn, affect the abundances of many other species. 
Second, as species with a high *f~D~*, they are also indispensable to fully control the plant-pollinator network. 
...
To this end, the two indices we employed to characterise a species provide two complementary pieces of information. 
(FDO revise grammar) Our first index, the frequency with which a species is classified as a driver node (*f~D~*) provides an indication of the likelihood that a species is part of the set of species that need to be manipulated in order to control of *all* species in the community. 
...
First, unlike some other types of complex systems, fully controlling an ecological community is almost certainly out of reach for all but the simplest ones, due to either the number of required interventions or the difficulties of their implementation [@Motter2015]. 
For instance, our results suggest that full control of the pollination networks would require direct interventions on 40-90% of the species.
...
In fact, because superior nodes are invariably at the beginning of a matched link, species with a high *f~S~* are more likely to be the subjects of management interventions when controlling the abundances of a target set of species—as opposed to the entire network—is desired [@Gao2014]. 
...
(FDO check) Despite the apparent similarities, our approach is different to previous attempts to quantify species importance. 
...
(FDO check) By allowing for the fact that some are more optimal than others in different contexts, control theory implicitly highlights that management decisions should be based on a single technique. 
