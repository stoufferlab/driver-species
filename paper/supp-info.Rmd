...
```{r tab-controllabillity-model-results, results = "asis", sanitize = FALSE}
...
```{r tab-species-model-random, warning=FALSE}
...
dplyr::bind_rows(cc_imp, su_imp) %>%
  ...
  knitr::kable("latex", 
               ...
               digits = 2, 
               escape = F, 
               caption = "Variable importance for each set of models. The importance is the sum of the weights of all models in which the variable was included.",
               linesep = '') %>%
  kableExtra::kable_styling(font_size = 8)%>%
...
```{r fig-species-models-degree, fig.width=fig_sizes()[[2]], fig.height=fig_sizes()[[1]]/2, fig.cap = "Relationship between degree and (a) the control capacity and (b) the probability of being a superior node. The plots show the values predicted by most parsimonious model for each response variable. The invasive species are depicted with solid orange circles."}
drake::loadd(fig_models_degree)
library(ggplot2)
fig_models_degree %<>% add_internal_margins()
cowplot::plot_grid(plotlist = fig_models_degree, ncol = 2)
```
